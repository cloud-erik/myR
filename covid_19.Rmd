...
Keep in population only data from last year and current variant "Medium".
...
#plot map overlay
ggmap(nMap)+coord_map(xlim=xlim,ylim=ylim) +
    #geom_polygon(data=mapdata, aes(x=X, y=Y, group=PID, fill=death), alpha=0.9) +
...
#plot map overlay
ggmap(nMap)+coord_map(xlim=xlim,ylim=ylim) +
    #geom_polygon(data=mapdata, aes(x=X, y=Y, group=PID, fill=recovered), alpha=0.9) +
...
And of course the values are absolute and have to be weighted or set in correlation of the population of the country.
